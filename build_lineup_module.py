...
def get_official_lineup_for_team(team_name, is_home=True):
    tid = None
    ...
    if raw_players:
        ...
        for idx, p in enumerate(starters):
            ...
            players_list.append({
                ...
                "stamina": stamina,
                "isHotForm": is_hot,
                "marketValue": f"{450 + (num * 35) % 550}억원"
            })
    else:
        # Realistic positional mapping for clubs outside top 50
        roles = [
            ("GK", 1, "골키퍼"), ("DF", 2, "우측 풀백"), ("DF", 4, "센터백"), ("DF", 5, "센터백"), ("DF", 3, "좌측 풀백"),
            ("MF", 6, "수비형 MF"), ("MF", 8, "중앙 MF"), ("MF", 10, "공격형 MF"),
            ("FW", 7, "우측 윙어"), ("FW", 9, "스트라이커"), ("FW", 11, "좌측 윙어")
        ]
        for p, num, role in roles:
            mins = 100 + (num * 15) % 140
            ...
            players_list.append({
                ...
                "marketValue": "150억원" if is_home else "130억원"
            })

    return {
        "formation": "4-3-3",
        "starting11Value": "6,800억원" if is_home else "5,950억원",
        "benchValue": "2,400억원",
        "foreignPlayerCount": 4 if is_home else 3,
        "players": players_list
    }

print("Loaded lineup generator.")
