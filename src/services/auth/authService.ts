...
export class AuthService {
      ...
      if (window.Kakao) {
        try {
          if (!window.Kakao.isInitialized || !window.Kakao.isInitialized()) {
            ...
            console.log('[AuthService] Kakao SDK Initialized with key:', this.kakaoJsKey);
          }
        } catch (e) {
  ...
  /**
   * 💬 카카오톡 공식 1초 실제 로그인 (팝업 실패 시 Direct OAuth 자동 전환)
   */
  public async loginWithKakao(): Promise<UserSessionData> {
    ...
    return new Promise((resolve, reject) => {
      ...
      // 1순위: SDK v1 Kakao.Auth.login 시도
      if (window.Kakao && window.Kakao.Auth && typeof window.Kakao.Auth.login === 'function') {
        try {
          window.Kakao.Auth.login({
            ...
            fail: (err: any) => {
              console.warn('[AuthService] Kakao.Auth.login popup failed, switching to direct OAuth redirect:', err);
              // 팝업 실패 시 Direct Kakao OAuth URL로 자동 이동
              const kakaoAuthUrl = `https://kauth.kakao.com/oauth/authorize?client_id=${this.kakaoJsKey}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code`;
              window.location.href = kakaoAuthUrl;
            }
          });
          return;
        } catch (callErr) {
          console.warn('[AuthService] Kakao.Auth.login execution error, switching to direct OAuth:', callErr);
        }
      }

      // 2순위: Direct Kakao OAuth URL 리다이렉트
      const kakaoAuthUrl = `https://kauth.kakao.com/oauth/authorize?client_id=${this.kakaoJsKey}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code`;
      window.location.href = kakaoAuthUrl;
    });
  }
