    ...
    <header className={`sticky top-0 z-40 backdrop-blur-xl border-b transition-colors duration-200 w-full ${
      ...
      {activeTab === 'home' && (
        <div className={`border-t w-full ${
          ...
          <div className="max-w-7xl mx-auto px-2 py-2 flex flex-col gap-1.5 w-full">
            {/* 1. Main 4 Betman Game Types (2x2 on mobile, 4-inline on desktop) */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-1.5 w-full">
              {/* 🎟️ 프로토 승부식 + 회차 선택 드롭다운 */}
              <div
                onClick={() => onSelectFolder('SEUNGBUSHIK')}
                className={`w-full px-2.5 py-1.5 rounded-xl text-[11px] font-black transition-all cursor-pointer flex items-center justify-between gap-1 border shadow-xs ${
                  selectedFolder === 'SEUNGBUSHIK' || selectedFolder === 'ALL'
                    ? 'bg-amber-400 text-slate-950 border-amber-300'
                    : isLight ? 'text-slate-700 hover:text-slate-950 bg-white border-slate-200' : 'text-slate-300 hover:text-white bg-slate-900 border-slate-800'
                }`}
              >
                <span className="truncate">🎟️ 승부식</span>
                <select
                  ...
                  className="bg-amber-300/90 hover:bg-amber-200 text-amber-950 border border-amber-500/80 rounded-md px-1.5 py-0.5 text-[10px] font-black outline-none cursor-pointer shrink-0"
              ...
              >
                <span className="truncate">⚽ 승무패</span>
                <select
                  ...
                  onClick={(e) => e.stopPropagation()}
                  className="bg-emerald-600 hover:bg-emerald-700 text-white border border-emerald-400/80 rounded-md px-1.5 py-0.5 text-[10px] font-black outline-none cursor-pointer shrink-0"
              ...
              >
                <span className="truncate">⚾ 승1패</span>
                <select
                  ...
                  onClick={(e) => e.stopPropagation()}
                  className="bg-orange-600 hover:bg-orange-700 text-white border border-orange-400/80 rounded-md px-1.5 py-0.5 text-[10px] font-black outline-none cursor-pointer shrink-0"
              ...
              >
                <span className="truncate">🎯 기록식</span>
                <select
                  ...
                  onClick={(e) => e.stopPropagation()}
                  className="bg-purple-700 hover:bg-purple-800 text-white border border-purple-400/80 rounded-md px-1.5 py-0.5 text-[10px] font-black outline-none cursor-pointer shrink-0"
            ...
            {/* 2. Secondary Filter Options Bar (진행예정만 토글 & 정밀상세 토글 & 휴식기 종목) */}
            <div className="flex items-center gap-1.5 w-full">
              {/* 🌐 전체포함 / ⏰ 진행예정만 */}
              <button
                onClick={() => setHidePassedMatches(!hidePassedMatches)}
                className={`flex-1 py-1.5 px-2 rounded-xl text-[11px] font-black transition-all flex items-center justify-center gap-1.5 cursor-pointer border shadow-xs ${
                  hidePassedMatches
                    ? isLight ? 'bg-emerald-100 text-emerald-800 border-emerald-400' : 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40'
                    : isLight ? 'text-slate-700 hover:text-slate-950 bg-white border-slate-200' : 'text-slate-300 hover:text-white bg-slate-900 border-slate-800'
                }`}
              ...
              <button
                ...
                className={`hidden sm:flex px-2.5 py-1.5 rounded-xl text-[10px] font-bold transition-all cursor-pointer border shrink-0 ${
                  selectedFolder === 'SEUNG5PAE'
                    ? 'bg-slate-200 text-slate-800 border-slate-300'
                    : isLight ? 'text-slate-400 bg-slate-100 border-slate-200' : 'text-slate-500 bg-slate-900/40 border-slate-800/50'
                }`}
