...
def test_xg_direct_under_pitch():
    ...
    try:
        ...
        if detail_buttons:
            ...
            # Scroll down to show pitch bottom and xG section
            driver.execute_script("""
                const modalBody = document.querySelector('.overflow-y-auto, [class*="overflow-y"]');
                if (modalBody) {
                    modalBody.scrollTop = 950;
                }
            """)
            time.sleep(1.5)
            driver.save_screenshot("verified_xg_directly_under_pitch.png")
            print("Saved verified_xg_directly_under_pitch.png successfully!")
