...
def test_bournemouth_modal():
    ...
    try:
        ...
        print(f"Found {len(detail_buttons)} detail buttons in G011.")
        if detail_buttons:
            driver.execute_script("arguments[0].click();", detail_buttons[0])
            time.sleep(1.5)
            driver.save_screenshot("verified_bournemouth_modal.png")
            print("Saved verified_bournemouth_modal.png successfully!")
